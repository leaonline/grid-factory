 ...
 * @return {*}
 */
export const getCheckUser = ({
  validateUser,
  log = () => {},
  onErrorHook = (err) => console.error(err),
  i18nFactory = (x) => x,
}) => {
  ...
  return async (context, file, type) => {
    let hasPermission
    const ctx = context ?? {}

    try {
      log('checkUser', ctx.user, ctx.userId, type)
      ...
      let user = typeof ctx.user === 'function' ? ctx.user() : ctx.user
      ...
      hasPermission = validateUser
        ? await validateUser(user, file, type, i18nFactory)
        : true
