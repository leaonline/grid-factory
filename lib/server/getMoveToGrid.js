...
export const getMoveToGrid = ({ bucket, fs, log, debug }) => {
  ...
  return function moveToGrid (file, Collection, filesCollection) {
    ...
    const promises = Object.entries(file.versions || {}).map(([versionName, version = {}]) => {
      return new Promise((resolve, reject) => {
        log(`move ${file._id}/${file.name} (${versionName}) to bucket [${bucket.name}]`)
        ...
        log(`upload stream for ${file._id}/${file.name} (${versionName}) is ${gridFileId}`)
        uploadStream.on('finish', async () => {
            log(`finish ${file._id}/${file.name} (${versionName}) in bucket [${bucket}]`)
        ...
        uploadStream.on('error', async (err) => {
            log(`failed ${file._id}/${file.name} (${versionName}) in bucket [${bucket}]`)
            console.error(err);
            await unlink(versionName)
            reject(err)
          });

        const readStream = fs.createReadStream(file.versions[versionName].path)
